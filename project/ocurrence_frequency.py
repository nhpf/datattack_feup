...
def ocurrence_frequency(
    df: pd.DataFrame, categorylevel: str, geoscope: str, datasetisclean=False
):
    ...
    geo_list = []
    abreviation_dict = {
        "Assistência e Prevenção a actividades humanas": 1,
        "Comprometimento total ou parcial de segurança, serviços ou estruturas": 2,
        "Incêndios Urbanos ou em Área Urbanizável": 3,
        "Incêndios em Equipamento e Produtos": 4,
    }
        ...
        for row2 in df[geoscopef]:
            if row1 == row2:
                ...
                if category in cat_list:
                    ...
                    cat_list[cat_list.index(category) + 1] = (
                        cat_list[cat_list.index(category) + 1] + 1
                    )
                else:
    ...
    list_of_tuples = [
        [(lst[2 * i], lst[2 * i + 1]) for i in range(round(len(lst) / 2 - 1))]
        for lst in geo_list
    ]
    ...
    for row1 in df[geoscopef].unique():
        ...
        plt.savefig(
            os.path.join(
                ".", "images", f"{row1}_ocurrences_{geoscope}_{categorylevel}.png"
            )
        )
        plt.clf()
        plt.cla()
